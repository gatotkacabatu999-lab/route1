...
type KmCalculationMode = "starting-point" | "step-by-step"
...
export function HomeContent({ initialRouteId }: HomeContentProps) {
  ...
  const [kmCalcMode, setKmCalcMode] = React.useState<KmCalculationMode>("starting-point")
  ...
  function getKmValueForAssignment(assignment: RouteLocation): number | null {
    if (kmCalcMode === "starting-point") {
      if (customStart) {
        return liveKmCache[assignment.locationCode] ?? assignment.km ?? null
      }
      return assignment.km ?? null
    }

    if (kmCalcTotals[assignment.locationCode] != null) {
      return kmCalcTotals[assignment.locationCode]
    }

    return assignment.km ?? null
  }

  function handleApplyKmMode() {
                    ...
                    return (
                      ...
                      >
                        ...
                        {columnVisibility.km && (
                          <TableCell className="py-2.5 px-5 text-center font-medium tabular-nums">
                            {(() => {
                              const displayValue = getKmValueForAssignment(assignment)
                              if (displayValue == null) {
                                return <span className="text-muted-foreground/40">—</span>
                              }
                              if (kmCalcMode === "starting-point" && customStart && !(assignment.locationCode in liveKmCache)) {
                                return <span className="text-muted-foreground/40 text-[10px]">…</span>
                              }
                              return <span>{formatKm(displayValue)}</span>
                            })()}
                          </TableCell>
                        )}
                      </TableRow>
        ...
        <DialogContent className="flex max-h-[80vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-lg">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
            {!selectedRoute ? (
              ...
              <div className="flex flex-col gap-3">
                <FieldDescription>
                  Pick the KM method for the route table. This selection will change how the route table displays KM values.
                </FieldDescription>
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  {[
                    {
                      id: "starting-point",
                      label: "By Starting Point",
                      description: "Use the selected start point for each location distance.",
                    },
                    {
                      id: "step-by-step",
                      label: "Step by Step",
                      description: "Use the saved step-by-step KM values for the route display.",
                    },
                  ].map((option) => {
                    const isActive = kmCalcMode === option.id
                    return (
                      <button
                        key={option.id}
                        type="button"
                        onClick={() => setKmCalcMode(option.id as KmCalculationMode)}
                        className={cn(
                ...
                <div className="rounded-lg border bg-muted/40 p-3 text-xs text-muted-foreground">
                  {kmCalcMode === "starting-point" ? (
                    <span>The route table will use the selected starting point for the KM column.</span>
                  ) : (
                    <span>The route table will use the saved step-by-step KM values for the route.</span>
                  )}
                </div>

                <Button type="button" size="sm" onClick={handleApplyKmMode} className="gap-1.5 self-end">
